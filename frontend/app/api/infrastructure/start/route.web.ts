...
import { startInstance } from '@/lib/aws-infra';
...
export async function POST() {
  ...
  try {
    await startInstance(config.aws.roleArn, config.aws.region, config.aws.instanceId);
    return Response.json({ success: true, message: 'Instance starting' });
  } catch (error: any) {
